  ...
  async function handleTranslate() {
    ...
    try {
      ...
      const res = await fetch("/api/translate/text", {
        ...
        body: JSON.stringify({ lines, contextSummary: "", targetLanguage: getLanguageName(targetLanguage) }),
        signal: abortRef.current.signal,
      });

      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        toast.error(res.status === 402 ? "Character limit reached. Please top up your credits." : (data.error ?? "Translation failed"));
        return;
      }
