...
// Quick PDF text extraction for term detection (no rendering, first 3 pages max)
async function scanPdfText(file: File): Promise<string> {
  ...
  if (!win.pdfjsLib) {
    const cdns = [
      { main: "https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/legacy/build/pdf.min.js", worker: "https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/legacy/build/pdf.worker.min.js" },
      { main: "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js", worker: "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js" },
      { main: "https://unpkg.com/pdfjs-dist@3.11.174/legacy/build/pdf.min.js", worker: "https://unpkg.com/pdfjs-dist@3.11.174/legacy/build/pdf.worker.min.js" },
    ];
  ...
  try {
    ...
    const pdf = await (lib.getDocument({ data: new Uint8Array(arrayBuffer) }).promise) as {
      numPages: number;
    ...
    for (let i = 1; i <= maxPages && text.length < 4000; i++) {
...
export default function TranslatePage() {
  ...
  async function handleTranslate() {
    ...
    // Scan document for technical terms
    setScanning(true);
    let terms: string[] = [];
    try {
      if (isPdf(file)) {
        const text = await scanPdfText(file);
        if (text.trim()) {
          ...
          if (res.ok) terms = (await res.json()).terms ?? [];
        }
      } else {
        const fd = new FormData();
        fd.append("file", file);
        const res = await fetch("/api/translate/scan", { method: "POST", body: fd });
        if (res.ok) terms = (await res.json()).terms ?? [];
      }
    } catch { /* scan failed — proceed without terms */ }
    setScanning(false);
  ...
  async function startTranslation(terms: string[]) {
    ...
    try {
      ...
      if (terms.length > 0) formData.append("translateTerms", JSON.stringify(terms));
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        {stage === "translating" && file && isPdf(file) && (
          <PdfTranslatorClient
            ...
            translateTerms={translateTerms.length > 0 ? translateTerms : undefined}
